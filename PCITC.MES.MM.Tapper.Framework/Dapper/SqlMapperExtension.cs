...
namespace PCITC.MES.MM.Tapper.Framework.Dapper
{
    public static class SqlMapperExtension
    {
        ...
        {
            return connection.QueryAsync<T>(BuildSql(condition, table, columns, isOr), condition, transaction, commandTimeout);
        }

        private static string BuildSql(dynamic condition, string table, string selectPart = "*", bool isOr = false)
        {
            var conditionObj = condition as object;
