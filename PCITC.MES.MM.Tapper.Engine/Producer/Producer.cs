    ...
    public class Producer
    {
        ...
        private void GenerateTaskFromRules()
        {
            var rules =
                _modelService.GetRuleModels()
                    .Values.Where(r =>r.IsHeader==1 && r.RuleBegDate <= DateTime.Today && r.RuleEndDate > DateTime.Today).ToList();
            var childRules =
                _modelService.GetRuleModels()
                    .Values.Where(r => r.IsHeader == 0 && r.RuleBegDate <= DateTime.Today && r.RuleEndDate > DateTime.Today).ToList();
            ...
            foreach (var rule in filtedRules)
            {
                ...
                if (entity != null)
                {
                    ...
                    _logger.Info("生成新的链表Header任务,等待调度:{0}",entity.ToString());
                    GenerateChildTaskFromRules(rule.RuleIdNext, childRules, tasks.Values.ToList());
                }
            }
        }

        private void GenerateChildTaskFromRules(int nextRuleId,List<RuleModel> childRules,List<TaskModel> taskModels)
        {
            if (nextRuleId == 0 || childRules.Count==0) return;
            var rule = childRules.FirstOrDefault(r => r.RuleId == nextRuleId);
            if (rule == null)
            {
                _logger.Error("没有找到指定叶子规则模型，链表中断:RuleId={0}", nextRuleId);
                return;
            }
            var task = taskModels.FirstOrDefault(t => t.TaskId == rule.TaskId);
            if (task == null)
            {
                _logger.Error("没有找到指定叶子任务模型，链表中断:TaskId={0}",rule.TaskId);
                return;
            }
            var entity = GetEntityFromModel(rule, task);
            if (entity == null) return;
            entity.TaskState=TaskState.Created;
            IList<int> queueIds;         //指定topic下queue id列表
            _topicQueueIdsDict.TryGetValue(rule.Topic, out queueIds);
            if (queueIds == null)
            {
            ...
            var selectedQueueId = _queueSelector.SelectQueueId(queueIds, null, null);
            _taskService.AddTaskToQueue(entity, selectedQueueId);
            _logger.Info("生成新的链表任务,等待激活:{0}", entity.ToString());
            GenerateChildTaskFromRules(rule.RuleIdNext, childRules, taskModels);
        }
        ...
        private TaskEntity GetEntityFromModel(RuleModel rule, TaskModel task)
        {
            ...
            return new TaskEntity
            {
                ...
                IsHeader = rule.IsHeader,
                RuleIdNext = rule.RuleIdNext
            };
        }
