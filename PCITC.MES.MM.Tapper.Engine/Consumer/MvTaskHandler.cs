...
namespace PCITC.MES.MM.Tapper.Engine.Consumer
{
    public class MvTaskHandler : ITaskHandler
    {
        ...
        public dynamic TaskExcute(string methodName, Dictionary<string, string> methodParams)
        {
            var paramArray = GetParametersArray(methodName, methodParams);
            dynamic result = WcfChannelFactory.ExecuteMetod<IAuthorizeService>(ServiceUrl, ServiceBinding,
        ...
        {
            var method = typeof(IAuthorizeService).GetMethod(methodName);
